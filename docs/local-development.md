...
2. Create the `passion_cosmetic` and isolated `passion_cosmetic_test` databases.
...
`php artisan test` uses `passion_cosmetic_test` and the `pc_test:` Memurai prefix configured in `phpunit.xml`; never point tests at the development database.
