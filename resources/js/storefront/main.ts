...
if (detail) {
    ...
    detail.querySelectorAll<HTMLButtonElement>('[data-option-value]').forEach((button) => button.addEventListener('click', () => {
        ...
        const available = Boolean(variant?.is_active && variant.stock_quantity > 0); variantId = available ? variant?.public_id ?? null : null; if (addButton) addButton.disabled = !available;
    }));
...
async function renderCheckout(host: HTMLElement) {
    ...
    try {
        ...
        const promo = fields.meta.promo_code_field_visible ? '<fieldset class="promo-field"><legend>Code promo</legend><div><input name="promo_code" maxlength="80"><button type="button" data-promo-apply>Appliquer</button></div><p data-promo-message aria-live="polite"></p></fieldset>' : '';
        host.innerHTML = `<div class="checkout-layout"><form class="checkout-form" data-order-form novalidate><div class="form-errors" data-form-errors aria-live="assertive"></div>${fields.data.map(checkoutField).join('')}${promo}<p class="privacy-note">Vos informations servent uniquement à traiter et confirmer votre commande.</p><button class="button button-dark" type="submit">Confirmer la commande</button></form><aside class="cart-summary" data-checkout-summary>${checkoutSummary(quoted)}</aside></div>`;
        const form = host.querySelector<HTMLFormElement>('[data-order-form]');
        form?.querySelector('[data-promo-apply]')?.addEventListener('click', async () => { const input = form.elements.namedItem('promo_code') as HTMLInputElement; const message = form.querySelector<HTMLElement>('[data-promo-message]'); try { const updated = await quote(items, input.value.trim()); const summary = host.querySelector<HTMLElement>('[data-checkout-summary]'); if (summary) summary.innerHTML = checkoutSummary(updated); if (message) message.textContent = 'Code promo appliqué.'; } catch (cause) { if (message) message.textContent = cause instanceof Error ? cause.message : 'Code promo invalide ou indisponible.'; } });
