...
function saveCart(items: CartItem[]) { try { window.localStorage?.setItem(CART_KEY, JSON.stringify({ version: 2, expiresAt: Date.now() + CART_TTL, items })); } catch { /* The cart remains usable for this page when browser storage is blocked. */ } updateCartCount(true); }
...
function applyOptimisticCartUpdate(host: HTMLElement, quoteData: Quote, position: number, quantity: number) {
    ...
    const quantityChange = quantity - line.quantity_requested; const subtotal = quoteData.data.pricing.subtotal.millimes + (line.effective_unit_price.millimes * quantityChange); const total = quoteData.data.pricing.total.millimes + (line.effective_unit_price.millimes * quantityChange);
    const subtotalNode = host.querySelector<HTMLElement>('[data-cart-subtotal]'); if (subtotalNode) subtotalNode.textContent = formatMillimes(subtotal);
    ...
    const totalNode = host.querySelector<HTMLElement>('[data-cart-total]'); if (totalNode) totalNode.textContent = formatMillimes(total);
...
async function reconcileCart(host: HTMLElement, request: number) {
    ...
    try { const quoted = await quote(items); if (request === cartRenderRequest) { lastConfirmedCart = items.map((item) => ({ ...item })); renderQuotedCart(host, quoted); } } catch (cause) { if (request !== cartRenderRequest) return; saveCart(lastConfirmedCart); host.innerHTML = `<p class="commerce-alert">${escapeHtml(cause instanceof Error ? cause.message : 'Panier indisponible.')}</p><button class="button button-outline" type="button" data-cart-retry>Réessayer</button>`; host.querySelector('[data-cart-retry]')?.addEventListener('click', () => { void renderCart(host); }); }
}
...
function checkoutOrderSummary(quoted: Quote): string { return `<section class="checkout-order-items" aria-label="Articles de votre commande"><h2>Votre sélection</h2>${quoted.data.items.map((line) => `<article>${line.image_url ? `<img src="${escapeHtml(line.image_url)}" alt="">` : ''}<div><strong>${escapeHtml(line.name)}</strong>${line.variant_label ? `<small>${escapeHtml(line.variant_label)}</small>` : ''}<small>Quantité : ${line.quantity_requested}</small></div><strong>${line.line_total.formatted}</strong></article>`).join('')}</section>${checkoutSummary(quoted)}`; }
