...
type Product = { public_id: string; name: string; category?: Category; regular_price_millimes: number; stock_quantity: number | null; active_variant_stock_quantity?: number | null; has_variants: boolean; is_active: boolean; updated_at: string; images: { path: string | null }[] };
...
async function api<T>(path: string): Promise<T> {
    ...
    if (!response.ok) throw new Error(response.status === 401 || response.status === 403 ? 'Votre session ne permet pas cet accès.' : 'Impossible de charger les produits pour le moment. Réessayez dans un instant.');
...
const ProductsView: Component = {
    ...
    setup() {
        const route = useRoute(); const page = ref<Page<Product> | null>(null); const categories = ref<Category[]>([]); const loading = ref(true); const error = ref('');
