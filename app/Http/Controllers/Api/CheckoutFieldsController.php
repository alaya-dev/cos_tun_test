...
class CheckoutFieldsController extends Controller
{
    public function __invoke(Request $request, CreateGuestOrderAction $orders): JsonResponse
    {
        ...
        return response()->json(['data' => $data, 'meta' => ['schema_version' => $orders->schemaVersion($data->all()), 'promo_code_field_visible' => false, 'request_id' => $request->attributes->get('request_id')]]);
    }
}
